...
const { ProductDamagedInWarehouseIncident } = require('.');
...
describe('domain/incidents/product-damaged-in-warehouse', () => {
  ...
  beforeEach(() => {
    ...
    itemRepository = {
      getItemsDamagedAfter: sinon.spy(async () => ['ITEM-001', 'ITEM-002'])
    };
    preprocessDeps = {
      itemRepository,
  ...
  describe('preprocessing', () => {
    it('should reject if product is not found', async () => {
      const incident = new ProductDamagedInWarehouseIncident(INCIDENT_VALUES);
    ...
    it('should reject if product does not belong to user', async () => {
      const incident = new ProductDamagedInWarehouseIncident({
        ...INCIDENT_VALUES,
    ...
    it('should reject if no damage was found', async () => {
      itemRepository.getItemsDamagedAfter = sinon.spy(async () => []);
      ...
      const [scubIds, minDate] = itemRepository.getItemsDamagedAfter.args[0];
      const expectedDay = dayjs().subtract(90, 'days').format('YYYY-MM-DD');
      expect(scubIds).to.eql(['SCUB-002']);
      expect(dayjs(minDate).format('YYYY-MM-DD')).to.eql(expectedDay);
    });

    it('should reject if all damages were already reported', async () => {
    ...
    it('should report only not already reported damages', async () => {
